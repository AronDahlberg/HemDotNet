...
namespace HemDotNetWebApi.Mapping
{
    ...
    .ForMember(dest => dest.Images, opt => opt.MapFrom(src => src.Images));
            ...
            // Allan
            CreateMap<AgencyCreateDto, RealEstateAgency>()
                .ForMember(dest => dest.RealEstateAgencyId, opt => opt.Ignore()) // ID is usually generated by the database
                .ForMember(dest => dest.RealEstateAgencyAgents, opt => opt.Ignore()); // This list is not part of the DTO

            CreateMap<RealEstateAgency, AgencyCreateDto>();
        }
    }
}
